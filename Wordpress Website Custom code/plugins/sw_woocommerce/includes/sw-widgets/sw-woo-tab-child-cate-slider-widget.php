<?php
/**
 * Name: Sw Woo Tab Child Cate Slider widget
 * Description: A widget that serves as an slideshow for developing more advanced widgets.
 */

 
class sw_woo_tab_child_cate_slider_widget extends WP_Widget {
	...
	function __construct() {
		/* Widget settings. */
		$widget_ops = array( 'classname' => 'sw_woo_tab_child_cate_slider', 'description' => __('Sw Woo Tab Child Cate Slider', 'sw_woocommerce') );

		/* Widget control settings. */
		$control_ops = array( 'width' => 300, 'height' => 350, 'id_base' => 'sw_woo_tab_child_cate_slider' );

		/* Create the widget. */
		parent::__construct( 'sw_woo_tab_child_cate_slider', __('Sw Woo Tab Child Cate Slider widget', 'sw_woocommerce'), $widget_ops, $control_ops );
				
		add_shortcode( 'woo_tab_child_cate_slider', array( $this, 'SC_WooTab' ) );
		...
		/* Ajax Call */
			if( version_compare( WC()->version, '2.4', '>=' ) ){
					add_action( 'wc_ajax_sw_tab_child_category', array( $this, 'sw_tab_child_category_callback' ) );
			}
			else {
				add_action( 'wp_ajax_sw_tab_category', array( $this, 'sw_tab_child_category_callback') );
				add_action( 'wp_ajax_nopriv_sw_tab_child_category', array( $this, 'sw_tab_child_category_callback') );
			}
	}
	...
	**/
	function SC_integrateWithVC(){
		...
		vc_map( array(
			"name" => __( "Sw Woo Tab Child Cate Slider", "sw_woocommerce" ),
			"base" => "woo_tab_child_cate_slider",
			"icon" => "icon-wpb-ytc",
			"class" => "",
			...
			"params" => array(
			 ...
			 array(
				...
				"holder" => "div",
				"class" => "",
				"heading" => __( "Banner Slide", "sw_woocommerce" ),
				"param_name" => "banner_slide",
				"value" => '',
				"description" => __( "Select banner slide", "sw_woocommerce" )
			 ),
			 array(
				"type" => "textfield",
				"holder" => "div",
			 ...
			 array(
				...
				"holder" => "div",
				"class" => "",
				"heading" => __( "Number items per slide", "sw_woocommerce" ),
				"param_name" => "item_row",
				"value" =>array(1,2,3),
			...
			array(
				...
				"param_name" => "layout",
				"value" => array( 'Layout Default' => 'default', 'Layout ChildCat' => 'child-category','Layout ChildCat 2' => 'child-category-2'),
			...
			array(
				...
				"holder" => "div",
				"class" => "",
				"heading" => __( "Title child cate", "sw_woocommerce" ),
				"param_name" => "title",
				"value" => "",
				"description" => __( "title for child cate ", "sw_woocommerce" ),
				"dependency" => array(
				        "element" => "layout",
				        "value" => "child-category-2"
				    )
			 ),
			 array(
				"type" => "textfield",
	...
	**/
	function SC_WooTab( $atts, $content = null ){
		...
		ob_start();		
		if( $layout == 'default' ){
			include( plugin_dir_path(dirname(__FILE__)).'/themes/sw-woo-tab-child-cate-slider/default.php' );
		}else if( $layout == 'child-category' ){
			include( plugin_dir_path(dirname(__FILE__)).'/themes/sw-woo-tab-child-cate-slider/child-category.php' );
		}else if( $layout == 'child-category-2' ){
			include( plugin_dir_path(dirname(__FILE__)).'/themes/sw-woo-tab-child-cate-slider/child-category-2.php' );
		}
	...
	function sw_tab_child_category_callback(){
			...
			$banner_slide = ( isset( $_POST["banner"] )  && $_POST["banner"] != '' ) ? $_POST["banner"] : '';
			...
			if( $list->have_posts() ) :
			if( $layout == 'child-category-2' ) :
		?>
		<div class="tab-content-slider clearfix">	
	...
	public function widget( $args, $instance ) {
		...
		extract($instance);
		if ( !in_array( 'sw_woocommerce/sw_woocommerce.php', apply_filters( 'active_plugins', get_option( 'active_plugins' ) ) ) ) { 
			_e('Please active sw_woocommerce plugin or install woomcommerce plugin first', 'sw_woocommerce');
			return false;
		}
	...
	protected function getTemplatePath($tpl='default', $type=''){
		$file = '/'.$tpl.$type.'.php';
		$dir 	=	plugin_dir_path(dirname(__FILE__)).'/themes/sw-woo-tab-child-cate-slider';
	...
	public function form( $instance ) {
		...
		<p>
			...
			<select class="widefat"
				...
				<option value="child-category" <?php if ($widget_template=='child-category'){?> selected="selected"
				<?php } ?>>
